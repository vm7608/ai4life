...
class VideoDataModule(pl.LightningDataModule):
    ...
    def create_dataset(
        ...
        return labeled_video_dataset(
            ...
            decoder=decoder,
            video_path_prefix=self.root_data_dir)

    def __dataloader(self, train: bool):
        """Train/validation loaders."""
        if train:
            dataset = self.create_dataset(data_path=self.train_csv_path, 
                                          clip_sampler=make_clip_sampler("random", self.clip_duration),
                                          video_path_prefix=self.root_data_dir,
                                          transform=self.train_transform)
        else:
            dataset = self.create_dataset(data_path=self.val_csv_path, 
...
class VideoModel(pl.LightningModule):
    ...
    def __build_model(self):
        ...
        # 2. Classifier:
        self.fc = layers[-1]
        self.fc.proj = nn.Linear(in_features=2048, out_features=self.num_classes, bias=True)
